#!/usr/bin/python3
"""
Script that creates a new view for the link between Place objects and
...
def get_place_amenities(place_id=None, amenity_id=None):
    '''This gets the amenities of a place with the given id'''
    if place_id:
...
def remove_place_amenity(place_id=None, amenity_id=None):
    '''This removes an amenity with a given id from a place with a given id'''
    if place_id and amenity_id:
        ...
        if not amenity:
            raise NotFound()
        place_amenity_link = list(filter(
            lambda x: x.id == amenity_id, place.amenities))
        if not place_amenity_link:
            raise NotFound()
        if storage_t == 'db':
            amenity_place_link = list(filter(
                lambda x: x.id == place_id, amenity.place_amenities))
...
def add_place_amenity(place_id=None, amenity_id=None):
    '''This adds an amenity with a given id to a place with a given id'''
    if place_id and amenity_id:
        place = storage.get(Place, place_id)
        if not place:
            raise NotFound()
